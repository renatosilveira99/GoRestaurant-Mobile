...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      const { data: apiData } = await api.get<Food>(`foods/${routeParams.id}`);

      apiData.formattedPrice = formatValue(apiData.price);

      setFood(apiData);

      const extrasWithQtds = apiData.extras.map(extra => ({
        ...extra,
        quantity: 0,
      }));
